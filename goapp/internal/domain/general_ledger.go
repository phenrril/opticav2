...
type GeneralLedgerEntry struct {
	ID          uint      `json:"id" gorm:"primaryKey"`                            // Generic ID, actual column name might be id_ingreso or id_egreso
	...
	UserID      uint      `json:"id_usuario,omitempty" gorm:"column:id_usuario"`   // If present in tables
	ClientID    uint      `json:"id_cliente,omitempty" gorm:"column:id_cliente"`   // If present in tables
	SaleID      uint      `json:"id_venta,omitempty" gorm:"column:id_venta"`       // If present in tables
}
